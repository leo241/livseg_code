    ...
    def train(self, net, lr=0.01, EPOCH=40, max_iter=500, save_iter=500, print_iter=100, first_iter=0,
        ...
        for epoch in range(EPOCH):
            ...
            for step, (x, y, y2) in enumerate(self.train_loader):
                ...
                loss = loss1 + loss_func2(output2, y2).to(torch.float) * 0.01
                # loss = loss1
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
...
if __name__ == '__main__':
    ...
    depth = 4 # Unet网络的深度
    EPOCH = 200
    net = UNet(num_classes=2,depth = depth,size = 256,rgb_channel = 1,initial_channel =8)
    # net.load_state_dict(torch.load(r'D:\study\pga\newtry0427\code practice\实验记录\unet4_T1_0.pth')) # 预训练载入
