...
def dir2pre(net,nii_dir,model_dir,gold_standard_dir = False,save_dir = False,standard = False,topk = 3):
    ...
    if gold_standard_dir:
        ...
        return dice,ravd,ans,dice_score(gt, post_pre, 1), RAVD(gt, post_pre), assd(gt, post_pre)
...
if __name__ == '__main__':
    ...
    tests = trains[0:5]
    # trains, vals, tests = domain_generalization_dataset(domain1='lianying', domain2='feilipu', ratio=[1, 1])
    ...
    model = r'D:\study\pga\newtry0427\code practice\实验记录\BayeSeg_deeplab_T1_1e-4.pth'
    dices = []
    ravds = []
    assds = []
    dices_ = []
    ...
    for test in tests:
        ...
        dice, ravd, ASSD,dice_, ravd_, ASSD_ = dir2pre(net,img,model,label,'tmp.nii.gz',standard=False,topk=topk)
        dices.append(dice)
        ravds.append(ravd)
        assds.append(ASSD)
        dices_.append(dice_)
